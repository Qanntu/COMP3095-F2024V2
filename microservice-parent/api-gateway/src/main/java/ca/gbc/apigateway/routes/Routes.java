        ...
        return GatewayRouterFunctions.route("product-service")
                .route(RequestPredicates.path("/api/product"), request -> {
                    ...
                    try {
                        ...
                        return ServerResponse.status(500).body("An error occurred when routing to product-service");
                    }
                })
                .build();
    }
        ...
        return GatewayRouterFunctions.route("order-service")
                .route(RequestPredicates.path("/api/order"), request -> {
                    ...
                    try {
                        ...
                        return ServerResponse.status(500).body("An error ocurred when routing to order-service");
                    }
                })
                .build();
    }
        ...
        return GatewayRouterFunctions.route("inventory-service")
                .route(RequestPredicates.path("/api/inventory"), request -> {
                    ...
                    try {
                        ...
                        log.error("Error ocurred while routing request: {}", e.getMessage(), e);
                        return ServerResponse.status(500).body("An error ocurred when routing to inventory-service");
                    }
                })
                .build();
    }
